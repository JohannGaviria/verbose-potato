...
@pytest.fixture
def valid_exp(faker: Faker) -> int:
    return int(faker.future_datetime().timestamp())


class TestAccessTokenPayloadVO:
    def test_should_create_payload_when_data_is_valid(
        self, valid_jti: UUID, valid_sub: UUID, valid_role: UserRoleEnum, valid_exp: int
    ) -> None:
        payload_vo = AccessTokenPayloadVO(
            jti=valid_jti, sub=valid_sub, role=valid_role, exp=valid_exp
        )
    ...
    def test_should_raise_exception_when_jti_is_none(
        self, valid_sub: UUID, valid_role: UserRoleEnum, valid_exp: int
    ) -> None:
    ...
    def test_should_raise_exception_when_sub_is_none(
        self, valid_jti: UUID, valid_role: UserRoleEnum, valid_exp: int
    ) -> None:
        with pytest.raises(InvalidAccessTokenPayloadException):
            AccessTokenPayloadVO(
                jti=valid_jti,
    ...
    def test_should_raise_exception_when_role_is_none(
        self, valid_jti: UUID, valid_sub: UUID, valid_exp: int
    ) -> None:
        with pytest.raises(InvalidAccessTokenPayloadException):
            AccessTokenPayloadVO(
                jti=valid_jti,
    ...
    def test_should_raise_exception_when_jti_is_not_a_uuid(
        self, jti: Any, valid_sub: UUID, valid_role: UserRoleEnum, valid_exp: int
    ) -> None:
        with pytest.raises(InvalidAccessTokenPayloadException):
            AccessTokenPayloadVO(jti=jti, sub=valid_sub, role=valid_role, exp=valid_exp)

    @pytest.mark.parametrize("sub", [123, "not-a-uuid", [], {}])
    def test_should_raise_exception_when_sub_is_not_a_uuid(
        self, sub: Any, valid_jti: UUID, valid_role: UserRoleEnum, valid_exp: int
    ) -> None:
        with pytest.raises(InvalidAccessTokenPayloadException):
            AccessTokenPayloadVO(jti=valid_jti, sub=sub, role=valid_role, exp=valid_exp)

    @pytest.mark.parametrize("role", [123, "MEMBER", [], {}])
    def test_should_raise_exception_when_role_is_not_a_user_role_enum(
        self, role: Any, valid_jti: UUID, valid_sub: UUID, valid_exp: int
    ) -> None:
        with pytest.raises(InvalidAccessTokenPayloadException):
            AccessTokenPayloadVO(jti=valid_jti, sub=valid_sub, role=role, exp=valid_exp)

    @pytest.mark.parametrize("exp", ["1735689600", 1735689600.5, [], {}])
    def test_should_raise_exception_when_exp_is_not_an_integer(
        self, exp: Any, valid_jti: UUID, valid_sub: UUID, valid_role: UserRoleEnum
    ) -> None:
        with pytest.raises(InvalidAccessTokenPayloadException):
            AccessTokenPayloadVO(jti=valid_jti, sub=valid_sub, role=valid_role, exp=exp)
    ...
    def test_should_be_immutable_when_created(
        ...
        with pytest.raises(FrozenInstanceError):
            payload_vo.exp = valid_exp + 60  # type: ignore[misc]
